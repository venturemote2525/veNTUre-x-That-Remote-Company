...
class FoodMaskRCNNConfig:
    ...
    def __init__(self, name="food_maskrcnn", **kwargs):
        ...
        # Base directories (module root)
        _base = _Path(__file__).resolve().parents[1]  # src/segmentation/mask_rcnn

        # Dataset configuration
        self.data_root = kwargs.get('data_root', str(_base / 'datasets' / 'swiss_coco_7class'))
        ...
        # Paths (scoped under module)
        self.checkpoint_dir = kwargs.get(
            'checkpoint_dir', str(_base / 'training' / 'checkpoints' / self.name)
        )
        self.log_dir = kwargs.get(
            'log_dir', str(_base / 'training' / 'logs' / self.name)
        )
        self.model_dir = kwargs.get(
            'model_dir', str(_base / 'models' / self.name)
        )
...
class Food7ClassConfig(FoodMaskRCNNConfig):
    ...
    def __init__(self, **kwargs):
        ...
        # Name reflects dataset + backbone
        name = f'swiss_7class_{backbone}'

        # Paths scoped to module
        kwargs.setdefault('log_dir', str(_base / 'training' / 'logs' / name))
        kwargs.setdefault('checkpoint_dir', str(_base / 'training' / 'checkpoints' / name))
        kwargs.setdefault('data_root', str(_base / 'datasets' / 'swiss_coco_7class'))
        kwargs.setdefault('model_dir', str(_base / 'models' / name))
