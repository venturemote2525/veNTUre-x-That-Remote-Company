...
def calculate_mAP(predictions, targets, num_classes, iou_threshold=0.5):
    ...
    try:
        ...
        for pred, target in zip(predictions, targets):
            # Predictions - ensure tensors
            pred_dict = {
                'boxes': torch.tensor(pred['boxes']) if isinstance(pred['boxes'], np.ndarray) else pred['boxes'],
                'scores': torch.tensor(pred['scores']) if isinstance(pred['scores'], np.ndarray) else pred['scores'],
                'labels': torch.tensor(pred['labels']) if isinstance(pred['labels'], np.ndarray) else pred['labels']
            }
            preds.append(pred_dict)
            
            # Targets - ensure tensors
            target_dict = {
                'boxes': torch.tensor(target['boxes']) if isinstance(target['boxes'], np.ndarray) else target['boxes'],
                'labels': torch.tensor(target['labels']) if isinstance(target['labels'], np.ndarray) else target['labels']
            }
