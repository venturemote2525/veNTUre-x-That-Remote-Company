            ...
            for batch_idx, (images, targets) in enumerate(self.val_loader):
                ...
                try:
                    ...
                    for i, (output, target) in enumerate(zip(outputs, targets)):
                        # Only keep high confidence predictions to reduce memory
                        if len(output['scores']) > 0:
                            valid_idx = output['scores'] > 0.01  # Very low threshold for early training
                            pred = {
                                'boxes': output['boxes'][valid_idx].cpu().numpy()[:10],  # Limit to top 10
                                'scores': output['scores'][valid_idx].cpu().numpy()[:10],
                                'labels': output['labels'][valid_idx].cpu().numpy()[:10]
                            }
                            ...
                            if batch_idx == 0:  # Only first batch to avoid spam
                                ...
                                print(f"Debug: Max score: {max_score:.4f}, Valid predictions: {num_valid}")
                        else:
...
def main():
    ...
    parser.add_argument('--data_root', type=str, 
                       default='../data/merged/rte8_7class_food_dataset',
